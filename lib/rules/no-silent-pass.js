 ...
 * `await`, which is only legal inside an async function — fixing a violation in
 * a synchronous callback would emit `await` in a sync scope (a SyntaxError on
 * `eslint --fix`). Returns false at module top level / unknown scope.
 */
function enclosingFnIsAsync(node) {
  let cur = node.parent;
  while (cur) {
    if (
    ...
    return {
      CallExpression(node) {
        ...
        context.report({
          ...
          // web-first default, and only inside an async function — injecting
          // `await` into a sync callback would be a SyntaxError on `--fix`.
          // Heuristic identifier matches and sync scopes stay report-only.
          fix(fixer) {
            if (!isInlineLocator(arg)) return null;
            if (!enclosingFnIsAsync(node)) return null;
            // If the assertion is already awaited (`await expect(loc).toBeTruthy()`,
            // a common real-world shape), reuse that `await` instead of prepending
            // another — otherwise the fix emits `await await expect(...)`.
            const alreadyAwaited = node.parent && node.parent.type === "AwaitExpression";
            const replacement = `${alreadyAwaited ? "" : "await "}expect(${argText}).toBeVisible()`;
